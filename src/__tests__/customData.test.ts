...
import { CustomDataHandler, CUSTOM_DATA_DELIMITER, encode, HybridType, KeyValueStoreStrategy, parse } from "../components/customData";
...
const generateValueStoreTest = <T extends CustomDataHandler>(customDataHandler: T) => async () => {
  ...
  const customId = await encode(componentId, customData, customDataHandler, requestId);

  const storedData = await parse(customId, customDataHandler);
...
test("Hybrid uses Simple strategy if smaller than max length.", async () => {
  ...
  const hybridCustomId = await encode(componentId, customData, hybridCustomDataHandler, requestId);
